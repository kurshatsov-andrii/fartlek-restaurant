...
const Customer = () => {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <ZoneHeader zone="CUSTOMER" title={`${tr.yourTable}: №${tableNo}`} subtitle="QR Menu" />
        ...
        <Tabs defaultValue="menu" className="w-full">
          ...
          <TabsContent value="menu" className="space-y-4">
            ...
            <div className="space-y-3">
              {filtered.map(d => (
                <div key={d.id} className="bg-card border border-border rounded-2xl p-4 flex gap-3 animate-fade-in">
                  <div className="h-20 w-20 rounded-xl bg-secondary flex items-center justify-center text-4xl flex-shrink-0">
                    {d.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-display font-bold leading-tight">{d.name[lang]}</h3>
                      {d.popular && <span className="text-[10px] bg-accent/15 text-accent px-2 py-0.5 rounded-full font-semibold">★ TOP</span>}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{d.description[lang]}</p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="font-display font-bold text-accent">{fmtUAH(d.price)}</span>
                      {cart[d.id] ? (
                        <div className="flex items-center gap-1 bg-secondary rounded-full">
                          <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full" onClick={() => sub(d.id)}><Minus className="h-3 w-3" /></Button>
                          <span className="text-sm font-semibold w-5 text-center">{cart[d.id]}</span>
                          <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full" onClick={() => add(d.id)}><Plus className="h-3 w-3" /></Button>
                        </div>
                      ) : (
                        <Button size="sm" onClick={() => add(d.id)} className="rounded-full h-8">
      ...
      {cartCount > 0 && (
        <Sheet>
          <SheetTrigger asChild>
            <Button className="fixed bottom-6 left-1/2 -translate-x-1/2 rounded-full h-14 px-6 shadow-elegant bg-foreground text-background hover:bg-foreground/90 z-50">
              <ShoppingBag className="h-5 w-5 mr-2" />
              <span className="font-semibold">{tr.cart} · {cartCount}</span>
              <span className="ml-3 pl-3 border-l border-background/20 font-bold">{fmtUAH(total)}</span>
            </Button>
          </SheetTrigger>
            ...
            <div className="space-y-3 mt-4">
              {cartItems.map(i => (
                <div key={i.dish.id} className="flex items-center gap-3 py-2 border-b border-border">
                  ...
                  <span className="font-bold">{fmtUAH(i.dish.price * i.qty)}</span>
              ...
              <div className="flex justify-between items-center pt-3">
                ...
                <span className="font-display font-bold text-2xl text-accent">{fmtUAH(total)}</span>
              </div>
      ...
      {placed && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-card border border-border rounded-3xl p-8 max-w-sm mx-4 text-center shadow-elegant">
...
const ReservationForm = () => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    toast.success('✓ ' + tr.reservation);
  };
  return (
    <form onSubmit={onSubmit} className="space-y-4 bg-card border border-border rounded-2xl p-5">
      <div>
        <Label>{tr.name}</Label>
